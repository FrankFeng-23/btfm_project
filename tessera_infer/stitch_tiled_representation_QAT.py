...
def stitch_and_crop_representations(d_pixel_retiled_path, representation_retiled_path, downstream_tiff_path, out_dir):
    """
    Optimized version: Stitch representation vectors and scales, then crop to the downstream TIFF extent.
    Output files maintain the same data type (dtype) as input files.
    
    Parameters:
    d_pixel_retiled_path: Directory path containing roi.tiff files
    representation_retiled_path: Directory path containing representation vector npy files and scales files
    ...
    # Analyze a sample representation vector to get the number of bands and dtype
    ...
    rep_dtype = rep_sample.dtype  # Get the dtype of representation arrays
    
    print(f"Representation vector dimensions: {patch_height} x {patch_width} x {num_bands}")
    print(f"Representation dtype: {rep_dtype}")
    
    # Check if scales file exists for the first folder
    first_scales_path = os.path.join(representation_retiled_path, f"{first_folder}_scales.npy")
    has_scales = os.path.exists(first_scales_path)
    scales_dtype = None
    if has_scales:
        scales_sample = np.load(first_scales_path)
        scales_dtype = scales_sample.dtype  # Get the dtype of scales arrays
        print(f"Scales dimensions: {scales_sample.shape[0]} x {scales_sample.shape[1]}")
        print(f"Scales dtype: {scales_dtype}")
    
    # Initialize target arrays with the same dtype as input
    target_array = np.zeros((downstream_height, downstream_width, num_bands), dtype=rep_dtype)
    target_scales_array = np.zeros((downstream_height, downstream_width), dtype=scales_dtype) if has_scales else None
    ...
    for patch_index, patch_info in tqdm(enumerate(intersecting_patches), total=len(intersecting_patches)):
        ...
        # Load corresponding scales data if it exists
        scales_data = None
        if has_scales:
            scales_path = os.path.join(representation_retiled_path, f"{folder}_scales.npy")
            if os.path.exists(scales_path):
                scales_data = np.load(scales_path)
            else:
                print(f"Warning: Scales file not found for {folder}, skipping this patch")
                continue
        ...
        if src_height != dst_height or src_width != dst_width:
            # Create temporary arrays for resampled data with appropriate dtype
            resampled_data = np.zeros((dst_height, dst_width, num_bands), dtype=rep_data.dtype)
            resampled_scales = np.zeros((dst_height, dst_width), dtype=scales_data.dtype) if scales_data is not None else None
            ...
            for y in range(dst_height):
                for x in range(dst_width):
                    ...
                    if y0 >= src_row_start and x0 >= src_col_start:
                        # Interpolate representation data
                        for b in range(num_bands):
                            # Perform interpolation in float, then convert back to original dtype
                            interpolated_value = (
                                wy0 * wx0 * float(rep_data[y0, x0, b]) +
                                wy0 * wx1 * float(rep_data[y0, x1, b]) +
                                wy1 * wx0 * float(rep_data[y1, x0, b]) +
                                wy1 * wx1 * float(rep_data[y1, x1, b])
                            )
                            # Convert back to original dtype with proper rounding for integers
                            if np.issubdtype(rep_data.dtype, np.integer):
                                resampled_data[y, x, b] = np.round(interpolated_value).astype(rep_data.dtype)
                            else:
                                resampled_data[y, x, b] = interpolated_value
                        
                        # Interpolate scales data if it exists
                        if scales_data is not None:
                            # Perform interpolation in float, then convert back to original dtype
                            interpolated_scale = (
                                wy0 * wx0 * float(scales_data[y0, x0]) +
                                wy0 * wx1 * float(scales_data[y0, x1]) +
                                wy1 * wx0 * float(scales_data[y1, x0]) +
                                wy1 * wx1 * float(scales_data[y1, x1])
                            )
                            # Convert back to original dtype with proper rounding for integers
                            if np.issubdtype(scales_data.dtype, np.integer):
                                resampled_scales[y, x] = np.round(interpolated_scale).astype(scales_data.dtype)
                            else:
                                resampled_scales[y, x] = interpolated_scale
            
            # Add resampled data to target arrays
            target_array[dst_row_start:dst_row_end, dst_col_start:dst_col_end, :] = resampled_data
            if scales_data is not None:
                target_scales_array[dst_row_start:dst_row_end, dst_col_start:dst_col_end] = resampled_scales
        else:
            ...
            # Add source region data to target arrays
            ...
            if scales_data is not None:
                target_scales_array[dst_row_start:dst_row_start+max_height,
                                    dst_col_start:dst_col_start+max_width] = scales_data[
                                    src_row_start:src_row_start+max_height,
                                    src_col_start:src_col_start+max_width]
    ...
    print(f"Saving representation to: {out_path}")
    np.save(out_path, target_array)
    
    # Save scales array if it exists
    if target_scales_array is not None:
        scales_out_path = os.path.join(out_dir, "stitched_scales.npy")
        print(f"Saving scales to: {scales_out_path}")
        np.save(scales_out_path, target_scales_array)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Stitch representation vectors and scales, then crop to downstream TIFF extent')
    parser.add_argument('--d_pixel_retiled_path', required=True, help='Directory path containing roi.tiff files')
    parser.add_argument('--representation_retiled_path', required=True, help='Directory path containing representation vector npy files and scales files')
...
"""
Usage:
python stitch_tiled_representation_QAT.py \
--d_pixel_retiled_path /home/zf281/rds/rds-sj514-data-WBrUDmBgqOo/jovana_gb/GA4/GA4_3/retiled_d_pixel \
--representation_retiled_path /home/zf281/rds/rds-sj514-data-WBrUDmBgqOo/jovana_gb/GA4/GA4_3/representation_retiled \
--downstream_tiff /home/zf281/rds/rds-sj514-data-WBrUDmBgqOo/jovana_gb/GA4/GA4_3/GA4_3.tif \
--out_dir /home/zf281/rds/rds-sj514-data-WBrUDmBgqOo/jovana_gb/GA4/GA4_3

Output files:
- stitched_representation.npy: Shape (H', W', C) - stitched representation vectors with same dtype as input
- stitched_scales.npy: Shape (H', W') - stitched scales with same dtype as input

Note: The output files will maintain the same data type (dtype) as the input files. For example, if input 
representation files are int8, the output stitched_representation.npy will also be int8.
"""