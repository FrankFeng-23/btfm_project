...
# Supports multiple processes per XPU for better resource utilization.
#
# Usage:
...
BASE_DATA_DIR="/home/zf281/rds/rds-sj514-data-WBrUDmBgqOo/biomassters_test"
...
# XPU Configuration
# Number of processes to run per XPU
PROCESSES_PER_XPU=4  # You can adjust this value (e.g., 2, 3, or 4)

# Define XPU batch size and workers
XPU_BATCH_SIZE=4096  # Larger for XPU, if this takes too much memory, reduce it
...
TILES_DIR="${BASE_DATA_DIR}/test_agbm_d_pixel"
OUTPUT_DIR="${BASE_DATA_DIR}/test_agbm_representation"
CONFIG_FILE="configs/multi_tile_infer_config.py"
PYTHON_SCRIPT="src/multi_tile_infer_QAT.py"
...
# Calculate total XPU processes
TOTAL_XPU_PROCESSES=$((XPU_COUNT * PROCESSES_PER_XPU))
log_info "Will run $PROCESSES_PER_XPU processes per XPU, total: $TOTAL_XPU_PROCESSES XPU processes"
...
if [ $XPU_TILES_COUNT -gt 0 ]; then
    ...
    if [ $XPU_COUNT -gt 0 ]; then
        log_info "Using $XPU_COUNT Intel XPUs with $PROCESSES_PER_XPU processes each (total: $TOTAL_XPU_PROCESSES processes)"
...
" || log_warning "Error getting XPU information"
        ...
        # Divide XPU tiles among all XPU processes
        if [ $XPU_TILES_COUNT -gt 0 ]; then
            TILES_PER_PROCESS=$((XPU_TILES_COUNT / TOTAL_XPU_PROCESSES))
            REMAINDER=$((XPU_TILES_COUNT % TOTAL_XPU_PROCESSES))
            
            log_info "Each XPU process will handle approximately $TILES_PER_PROCESS tiles"
            
            # Create tile lists for each XPU process
            start_idx=0
            process_counter=0
            
            for ((xpu_id=0; xpu_id<XPU_COUNT; xpu_id++)); do
                for ((proc_per_xpu=0; proc_per_xpu<PROCESSES_PER_XPU; proc_per_xpu++)); do
                    # Calculate how many tiles this process should handle
                    tiles_for_this_process=$TILES_PER_PROCESS
                    if [ $process_counter -lt $REMAINDER ]; then
                        tiles_for_this_process=$((tiles_for_this_process + 1))
                    fi
                    
                    # Calculate end index with bounds checking
                    end_idx=$((start_idx + tiles_for_this_process))
                    if [ $end_idx -gt $XPU_TILES_COUNT ]; then
                        end_idx=$XPU_TILES_COUNT
                    fi
                    
                    # Create unique identifier for this process
                    process_id="${xpu_id}_${proc_per_xpu}"
                    
                    # Create tile list for this process
                    TILE_LIST="src/tile_lists/tiles_xpu_${process_id}.json"
                    ...
                    log_info "Created tile list for XPU $xpu_id process $proc_per_xpu: $TILE_COUNT tiles"
            ...
            if [ $start_idx -ne $XPU_TILES_COUNT ]; then
                ...
                # Assign remaining tiles to the last process
                if [ $start_idx -lt $XPU_TILES_COUNT ] && [ $TOTAL_XPU_PROCESSES -gt 0 ]; then
                    last_xpu_id=$((XPU_COUNT - 1))
                    last_proc_id=$((PROCESSES_PER_XPU - 1))
                    process_id="${last_xpu_id}_${last_proc_id}"
                    TILE_LIST="src/tile_lists/tiles_xpu_${process_id}.json"
                    ...
                    log_info "Added remaining tiles to XPU $last_xpu_id process $last_proc_id"
                fi
            else
                log_success "All XPU tiles successfully assigned to processes"
            fi
        else
            log_warning "No tiles to process on XPU!"
            TOTAL_XPU_PROCESSES=0
        fi
    else
...
launch_xpu_processes() {
    if [ $XPU_TILES_COUNT -gt 0 ]; then
        ...
        log_info "Starting XPU processing with $TOTAL_XPU_PROCESSES processes across $XPU_COUNT XPUs"
                
        # Start XPU monitoring for each physical XPU
        for ((i=0; i<XPU_COUNT; i++)); do
            monitor_xpu_usage $i
            XPU_MONITOR_PIDS[$i]=$?
        done
        ...
        for ((xpu_id=0; xpu_id<XPU_COUNT; xpu_id++)); do
            for ((proc_per_xpu=0; proc_per_xpu<PROCESSES_PER_XPU; proc_per_xpu++)); do
                process_id="${xpu_id}_${proc_per_xpu}"
                TILE_LIST="src/tile_lists/tiles_xpu_${process_id}.json"
                LOG_FILE="logs/infer_xpu_${process_id}.log"
                ...
                if [ $TILE_COUNT -eq 0 ]; then
                    log_warning "Skipping XPU $xpu_id process $proc_per_xpu as it has no tiles assigned"
                    continue
                fi
                
                log_info "Starting XPU $xpu_id process $proc_per_xpu with $TILE_COUNT tiles"
                ...
                CMD="$PYTHON_ENV $PYTHON_SCRIPT --config $CONFIG_FILE --mode xpu --xpu_id $xpu_id --tile_list $TILE_LIST --process_id $process_id --checkpoint_path $CHECKPOINT_PATH --output_dir $OUTPUT_DIR --batch_size $XPU_BATCH_SIZE --num_workers $XPU_NUM_WORKERS --log_interval $LOG_INTERVAL $VERBOSE_XPU_FLAG --simplified_logging"
                
                # Start the XPU process
                nohup $CMD > "$LOG_FILE" 2>&1 &
                
                PID=$!
                log_info "XPU $xpu_id process $proc_per_xpu started with PID $PID"
                XPU_PIDS+=($PID)
                
                # Verify process started
                if ps -p $PID > /dev/null; then
                    log_success "XPU $xpu_id process $proc_per_xpu successfully started with PID $PID"
                else
                    log_error "Failed to start XPU $xpu_id process $proc_per_xpu"
                fi
                
                # Sleep briefly between launches to avoid resource contention
                sleep 2
            done
        done
    fi
...
monitor_xpu_processes() {
    if [ $XPU_TILES_COUNT -gt 0 ] && [ ${#XPU_PIDS[@]} -gt 0 ]; then
        log_info "Monitoring $TOTAL_XPU_PROCESSES XPU processes progress..."
        ...
        while [ $xpu_completed -lt ${#XPU_PIDS[@]} ]; do
            xpu_completed=0
            for pid in "${XPU_PIDS[@]}"; do
                if ! ps -p $pid > /dev/null 2>&1; then
            ...
            if [ $((current_time - last_check_time)) -ge $check_interval ]; then
                ...
                log_info "XPU progress: $xpu_completed/${#XPU_PIDS[@]} processes complete - Running for $elapsed"
                ...
                for ((xpu_id=0; xpu_id<XPU_COUNT; xpu_id++)); do
                    for ((proc_per_xpu=0; proc_per_xpu<PROCESSES_PER_XPU; proc_per_xpu++)); do
                        ...
                        # Check if log file exists and process is still running
                        if [ -f "$LOG_FILE" ] && [ -s "$LOG_FILE" ]; then
                            # Find corresponding PID (need to track which PID belongs to which process_id)
                            # For now, just check if the log has recent updates
                            echo "Latest progress from XPU $xpu_id process $proc_per_xpu:" >&2
                            # Find the latest batch progress lines
                            grep "Batch" "$LOG_FILE" | tail -n 2 || grep "Processing tile" "$LOG_FILE" | tail -n 1 || log_debug "No progress lines found for XPU $xpu_id process $proc_per_xpu"
                        fi
                    done
                done
                ...
                if [ $((current_time - last_detailed_time)) -ge $detailed_interval ]; then
                    log_info "Detailed XPU status update:"
                    for ((i=0; i<XPU_COUNT; i++)); do
            ...
            if [ $xpu_completed -eq ${#XPU_PIDS[@]} ]; then
                break
            fi
        ...
        for pid in "${XPU_PIDS[@]}"; do
            ...
            if [ $EXIT_CODE -eq 0 ]; then
                ...
                log_error "XPU process PID $pid failed with exit code $EXIT_CODE"
            fi
        done
...
log_info "  - XPU logs: logs/infer_xpu_*.log (format: infer_xpu_XPUID_PROCESSID.log)"
