...
def parse_args():
    parser = argparse.ArgumentParser(description="Multi-tile Inference with QAT (CPU/XPU)")
...
def process_tile(tile_path, output_path, model, config, device, process_id, args):
    ...
    try:
        ...
        # Inference loop
        local_results = []
        local_scales = []  # Store scales for each batch
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch_data in enumerate(loader):
                ...
                avg_repr_np = avg_repr.cpu().numpy()  # (B, latent_dim) in f32
                ...
                for b in range(B):
                    ...
                    repr_int8, scale = quantize_tensor_symmetric(repr_f32, bits=args.quantization_bits)
                    quantized_repr_list.append(repr_int8.numpy())
                    scales_list.append(scale.item())
                
                quantized_repr_np = np.stack(quantized_repr_list, axis=0)  # (B, latent_dim) in int8
                scales_np = np.array(scales_list, dtype=np.float32)  # (B,)
...
def main():
    ...
    log_dir = f"logs/node_{node_name}_job_{job_id}"
    ...
    try:
        import setproctitle
        if args.mode == 'cpu':
            setproctitle.setproctitle(f"infer_qat_cpu_{node_name}_{args.process_id}")
        else:
            setproctitle.setproctitle(f"infer_qat_xpu_{node_name}_{args.process_id}_{args.xpu_id}")
    except ImportError:
        pass
    ...
    try:
        ...
        logging.info(f"[{mode_prefix}] [{args.process_id}] Node: {node_name}, Job: {job_id}, {log_system_info()}")
        logging.info(f"[{mode_prefix}] [{args.process_id}] Quantization enabled with {args.quantization_bits} bits")
        ...
        if args.mode == 'cpu':
            ...
            if os.path.exists(output_path) and os.path.exists(output_path.replace('.npy', '_scales.npy')):
            ...
            for i, tile_path in enumerate(tile_list):
                ...
                scale_path = output_path.replace('.npy', '_scales.npy')
                
                # Skip if already processed
                if os.path.exists(output_path) and os.path.exists(scale_path):
