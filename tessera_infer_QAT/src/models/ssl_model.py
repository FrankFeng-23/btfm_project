...
class MultimodalBTInferenceModel(torch.nn.Module):
    ...
    Model for the inference phase, containing only two Transformer encoders (S2 + S1),
    without the projection head.
    Supports both f32 and quantized output.
    """
    def __init__(self, s2_backbone, s1_backbone, fusion_method, dim_reducer, output_quantized=False, quantization_bits=8):
        super().__init__()
        self.s2_backbone = s2_backbone
        self.s1_backbone = s1_backbone
        self.fusion_method = fusion_method
    ...
    def forward(self, s2_x, s1_x):
        ...
        # Return f32 representation during inference (quantization will be done outside)
        return fused