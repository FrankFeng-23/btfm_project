...
#   3) Convert S1 zips -> GeoTIFF in parallel (xargs -P25)
...
#   bash main_pipeline.sh /home/zf281/rds/rds-airr-p3-w8D3JcRiKZQ/cambridge/cambridge_shp/cambridge.tiff 2020-01-01 2021-01-01 2020-01-01 2021-01-01 1

########################################
# HOTSPOT LOG CLEANUP
########################################

# Function to clean up Hotspot logs
cleanup_hs_logs() {
    find . -name "hs_*.log" -type f -delete 2>/dev/null
}

# Initial cleanup at script start
cleanup_hs_logs

# Start background process to clean up logs every 60 seconds
(
    while true; do
        cleanup_hs_logs
        sleep 60
...
trap 'kill $CLEANUP_PID 2>/dev/null; cleanup_hs_logs; echo "Cleaned up Hotspot logs"' EXIT INT TERM
...
BASE_DATA_DIR="/home/zf281/rds/rds-airr-p3-w8D3JcRiKZQ/cambridge/2017"
TEMP_DATA_DIR="/local/zf281"
...
log_header() {
    echo -e "\n${BOLD}${BLUE}==== $1 ====${RESET}"
    # Clean up logs before starting a new section
    cleanup_hs_logs
}
...
mkdir -p ${BASE_DATA_DIR}/{data_processed,retiled_d_pixel,representation_retiled}
mkdir -p ${TEMP_DATA_DIR}/{data_sar,data_sar_processed,data_raw,snap_tmp,snap_auxdata}
log_info "Created necessary directories"

export S2_DATA_RAW="${TEMP_DATA_DIR}/data_raw"
export S2_DATA_PROCESSED="${BASE_DATA_DIR}/data_processed"
export S1_DATA_RAW="${TEMP_DATA_DIR}/data_sar"
export S1_DATA_PROCESSED="${TEMP_DATA_DIR}/data_sar_processed"
...
export SNAP_USER_DIR="${TEMP_DATA_DIR}/snap_auxdata"
export TMPDIR="${TEMP_DATA_DIR}/snap_tmp"
# Try to reduce Java logging by adding these options
export _JAVA_OPTIONS="-Djava.io.tmpdir=${TEMP_DATA_DIR}/snap_tmp"
export HOME_DEM_DIR="~/.snap/auxdata/dem/SRTM 1Sec HGT"

# Create wrapper function to run commands and clean logs afterward
run_cmd_and_clean() {
    "$@"
    local status=$?
    cleanup_hs_logs
...
export PYTHON="singularity exec \
  -B ${PARENT_DIR}:${PARENT_DIR} \
  -B ${BASE_SCRIPT_DIR}:/data \
  -B ${BASE_DATA_DIR}:${BASE_DATA_DIR} \
  -B ${TEMP_DATA_DIR}:${TEMP_DATA_DIR} \
  --env S2_DATA_RAW=${S2_DATA_RAW} \
  --env S2_DATA_PROCESSED=${S2_DATA_PROCESSED} \
  --env S1_DATA_RAW=${S1_DATA_RAW} \
  --env S1_DATA_PROCESSED=${S1_DATA_PROCESSED} \
  --env RUST_S2_EXE=${RUST_S2_EXE} \
  --env CLOUD_COVERAGE_THRESHOLD=${CLOUD_COVERAGE_THRESHOLD} \
  --env S1_DOWNLOAD_USERNAME=${S1_DOWNLOAD_USERNAME} \
  --env S1_DOWNLOAD_PASSWORD=${S1_DOWNLOAD_PASSWORD} \
  --env SNAP_USER_DIR=${SNAP_USER_DIR} \
  --env TMPDIR=${TMPDIR} \
  --env _JAVA_OPTIONS=${_JAVA_OPTIONS} \
  /home/zf281/rds/rds-sj514-data-WBrUDmBgqOo/btfm_project/singularity/snap-gdal-python.sif python3"
...
function build_s1_cmd() {
  ...
  echo "${PYTHON} s1_processor.py --zip_file \"${ZF}\" --reference_tiff \"${REF_TIF}\" --out_dir \"${OUT_DIR}\" && cleanup_hs_logs"
}
...
while read -r TILE_ID; do
  ...
  #   if [ "$NCMDS" -gt 0 ]; then
  #     log_info "Running S1 commands in parallel (-P25) and continuing even if some fail..."

  #     # Modified to continue processing even if some commands fail
  #     cat s1_commands_tile.txt | xargs -t -L1 -P25 -I{} bash -c '{} || echo "⚠️ Command failed with status $?: {} - but continuing..."'
      
  #     # Clean up logs after parallel execution is done
  #     cleanup_hs_logs
...
# Cleanup S1_DATA_RAW folder
log_info "Cleaning up S1_DATA_RAW folder"
rm -rf "${S1_DATA_RAW}"/*
...
log_info "Performing final cleanup of hotspot logs"
cleanup_hs_logs
