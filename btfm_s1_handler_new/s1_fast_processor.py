#!/usr/bin/env python3
"""
s1_fast_processor.py - Sentinel-1 RTC Fast Download & ROI Mosaic (Flexible Parallel Processing Version)
Updated: 2025-05-20
Supports flexible parallel partition processing with comprehensive error handling and timeout control
"""
...
PROCESS_TIMEOUT = 120 * 60  # overall timeout
DAY_TIMEOUT = 40 * 60      # single day processing timeout
ITEM_TIMEOUT = 20 * 60      # single item processing timeout
...
def search_items(bbox_ll, date_range:str, orbit_state="both", partition_id="unknown"):
    ...
    logging.info(f"[{partition_id}] STAC hits {len(items)} item")
    if items:
...
def process_item(item, tpl, bbox_proj, mask_np, resolution, chunksize, temp_dir, min_coverage, partition_id, retries=2):
    """Process single Sentinel-1 item and save as VV/VH two TIFF files, with retry mechanism
    ...
    for attempt in range(retries + 1):
        try:
            # Use timeout control for processing single item
            with timeout_handler(ITEM_TIMEOUT):
                # Use stackstac to load data
                ds = stackstac.stack(
                    [item], 
                    bounds=bbox_proj,
                ...
                # Compute data (trigger actual data loading)
                try:
                    # Explicitly compute data, this triggers actual remote reading
                    vv_values = vv_data.compute()
                    vh_values = vh_data.compute()
                except Exception as compute_error:
                    if attempt < retries:
                        logging.warning(f"[{partition_id}]   Attempt {attempt+1}/{retries+1} data computation failed: {compute_error}, retrying...")
                ...
                if vv_temp or vh_temp:
                    ...
                    # No files generated, but due to insufficient coverage, consider as skipped
                    return None, None, "skipped"
                
        except TimeoutException as e:
            if attempt < retries:
                logging.warning(f"[{partition_id}]   Attempt {attempt+1}/{retries+1} processing {date_str}_{orbit_state} timeout, retrying...")
                time.sleep(5)  # Wait 5 seconds before retry
                continue
            else:
                logging.error(f"[{partition_id}]   ⚠️ Processing {date_str}_{orbit_state} timeout: {e}")
            ...
            if is_retriable_error and attempt < retries:
                logging.warning(f"[{partition_id}]   Attempt {attempt+1}/{retries+1} processing {date_str}_{orbit_state} error: {type(e).__name__} - {e}, retrying...")
                time.sleep(3)  # Wait 3 seconds before retry
                continue
            else:
                logging.error(f"[{partition_id}]   ✗ Processing {date_str}_{orbit_state} error: {type(e).__name__} - {e}")
                return None, None, "failed"
    
    # If all retries failed
    logging.error(f"[{partition_id}]   ✗ Processing {date_str}_{orbit_state} all retries failed")
    return None, None, "failed"
...
def process_day_orbit(key, items, tpl, bbox_proj, mask_np, out_dir, resolution, chunksize, min_coverage, partition_id, overwrite=False):
    """Process all items for same date and orbit state, with 20-minute timeout control"""
    date_str, orbit_state = key.split("_")
    logging.info(f"[{partition_id}] → {key} (item={len(items)})")
    t0 = time.time()
    
    try:
        # Use 20-minute timeout control for single day processing
            ...
            try:
                ...
                for i, item in enumerate(items):
                    ...
                    if status == "success":
                        ...
                        skipped_count += 1
                        item_duration = time.time() - item_start_time
                        logging.info(f"[{partition_id}]   item {i+1} skipped (insufficient coverage or missing bands), took {item_duration:.1f}s")
                    else:  # status == "failed"
                        failed_count += 1
                        item_duration = time.time() - item_start_time
                ...
                logging.info(f"[{partition_id}]   items processing stats: success {processed_count}, skipped {skipped_count}, failed {failed_count} (total {len(items)})")
                
                # If no valid files, give different messages based on reason
                if not vv_temp_files and not vh_temp_files:
                    if processed_count == 0 and skipped_count > 0:
                        logging.info(f"[{partition_id}]   {key} all items skipped due to insufficient coverage or missing bands")
                        return True  # skipped is not failure
                    else:
...
def main():
    ...
    logging.info(f"[{args.partition_id}] ⚡ S1 Fast Processor starting (Flexible Parallel Edition)"); 
    log_sys(args.partition_id)
    logging.info(f"[{args.partition_id}] Processing timeout settings: overall {PROCESS_TIMEOUT//60}min, single day {DAY_TIMEOUT//60}min, single item {ITEM_TIMEOUT//60}min")
    ...
    # Search items based on orbit state
    ...
    if success_count == 0:
        ...
        logging.warning(f"[{args.partition_id}] ⚠️  Some dates failed processing ({total_count - success_count}/{total_count})")
